...
%% Path to data
path_data = '/Volumes/GoogleDrive/Shared drives/INVent/tom/Conferences/2022_SPIE/Data/Axial_resampled/';
...
folders = {[dataset 'RectalCA_']};
regions = {'tumor'}; %'Lumen' or 'ERW'
...
for ss = 1:length(folders)
    ...
    for rr = 1:length(regions)
        region = regions{rr};
        for i=1:10 % Move case by case - 1:116
            ...
            if exist(case_mask_path)
                ...
                if strcmp(folder,[dataset 'RectalCA_'])
                    if isunix
                        file_path = ['/Volumes/GoogleDrive/Shared drives/INVent/tom/Conferences/2022_SPIE/Shape_Features/2D_Feature_Matrices/Axial/All/' file_name]; % For Linux
                    elseif ispc
