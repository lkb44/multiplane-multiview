function [stats]= nFoldCV_withFS_v5c(data_set,data_labels,params)
...
%                   params.threshmeth: (optional) string of optimal ROC threshold method -- options: 'euclidean', 'matlab' (Default: 'matlab')
%
% OUTPUTS:
...
for j=1:params.nIter
  ...
  % *********STEP 1: FEATURE SELECTION*************%
        ...
        if strcmp(params.fsname, 'mrmr')
            ...
            training_set_d = Sdiscretize(training_set, [-inf -1 1 inf]);
            fea = mrmr_mid_d(training_set_d,training_labels,params.num_top_feats); % MRMR
            fea = fea'; 
        elseif strcmp(params.fsname, 'relieff')
            [fea,~] = relieff(training_set,training_labels,10);
            fea = fea(:,1:params.num_top_feats);
        elseif strcmp(params.fsname, 'lasso')
            B = lasso(training_set,training_labels);
            B_c = B(:,25); %the 25th Lambda value in B
            [~, lasso_idx] = sort(B_c,'descend');
            lasso_idx = lasso_idx';
            fea = lasso_idx(:,1:params.num_top_feats);
        else
            if isfield(params,'remouts') && ~isempty(params.remouts)
