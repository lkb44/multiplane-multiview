function [stats]= nFoldCV_withFS_v2(data_set,data_labels,params)
error('DO NOT USE. NEEDS TO BE UPDATED');

%***MODIFIED FOR ALLOWING AUGMENTED DATA***%
...
% 04-25-2017 - Modified to include option for handling multiple slices per patient ("augmented" data)
...
for j=1:params.nIter
    ...
    if isempty(params.subsets)
        if isfield(params,'patient_labels')
            [pts,idxs] = unique(params.patient_labels);
    ...
    for i=1:params.n
        ...
        if isfield(params,'patient_labels')
            training_set = data_set(ismember(params.patient_labels,pts(tra{i})),:);
            testing_set = data_set(ismember(params.patient_labels,pts(tes{i})),:);
            training_labels = data_labels(ismember(params.patient_labels,pts(tra{i})),:);
            testing_labels = data_labels(ismember(params.patient_labels,pts(tes{i})),:);
        else
  ...
  % *********STEP 1: FEATURE SELECTION*************%
        ...
        if isfield(params,'patient_labels')
            decision(ismember(params.patient_labels,pts(tes{i}))) = temp_stats.decision;
            prediction(ismember(params.patient_labels,pts(tes{i}))) = temp_stats.prediction;
        else
