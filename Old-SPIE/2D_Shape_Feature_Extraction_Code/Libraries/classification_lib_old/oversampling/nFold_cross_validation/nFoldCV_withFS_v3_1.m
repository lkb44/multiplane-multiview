...
%                   params.svm_kernel (optional, needed for 'SVM' only) --  options: 'linear','gaussian','polynomial')
...
if nargin == 3
    ...
    if strcmp(params.classifier,'SVM') && ~isfield(params,'svm_kernel'),...
            params.svm_kernel = 'linear';                   end
end
...
for j = 1:params.nIter
    ...
    if ~isfield(params,'subsets') || isempty(params.subsets)
        ...
        elseif ~isfield(params,'patients_ids') || isempty(params.patient_ids)
            smpts = [];
            lgpts = [];
            if params.n == length(params.fovs)
                [tra, tes] = GenerateSubsets('nFold',[],data_labels,params.shuffle,params.n);
            else
                for i = 1:length(params.fovs)
                    if strcmp(params.fovs(i), 'SmallFOV')
                        smpts = [smpts i];
                    elseif strcmp(params.fovs(i), 'LargeFOV')
                        lgpts = [lgpts i];
                    end
                end
                [traS, tesS] = GenerateSubsets('nFold',[],data_labels(smpts),params.shuffle,params.n);
                [traL, tesL] = GenerateSubsets('nFold',[],data_labels(lgpts),params.shuffle,params.n);
                for k = 1:params.n
                    %make sure large FOV is actual indicies of data_labels (add 19)
                    for n = 1:length(traL{k})
                        traL{k}(n) = traL{k}(n) + length(smpts);
                    end
                    
                    for n = 1:length(tesL{k})
                        tesL{k}(n) = tesL{k}(n) + length(smpts);
                    end
            ...
            if params.n == length(params.fovs)
            tra = params.subsets.training;
            tes = params.subsets.testing;
            else
        ...
        for i=1:params.n
            ...
            if strcmp(params.fsname, 'mrmr')
                ...
                if ~any(abs(mean(training_set))<1e-2) || ~any(abs(std(training_set))-1<1e-2)
            ...
            if strcmp(params.classifier,'SVM')
                [temp_stats,~] = Classify_wrapper(params.classifier, training_set(:,fea) , testing_set(:,fea), training_labels(:), testing_labels(:),params.svm_kernel);
            else
                [temp_stats,~] = Classify_wrapper(params.classifier, training_set(:,fea) , testing_set(:,fea), training_labels(:), testing_labels(:));
            end
        ...
        if numel(unique(testing_labels)) > 1 %won't work for Leave One Out CV
            ...
            %compute perfcurve across each testing fold per iteration
            try
                [X,Y,T,AUC,OPTROCPT] = perfcurve(foldlabels, foldpredictions, 1);
                ...
                [X,Y,T,AUC,OPTROCPT] = perfcurve(l(:), p(:), 1);
            end
