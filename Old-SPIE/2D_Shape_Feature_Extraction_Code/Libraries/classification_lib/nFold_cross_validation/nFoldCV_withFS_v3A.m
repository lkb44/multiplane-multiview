...
%%  Set default inputs
%mfilename='nFoldCV_withFS_v3A'
%add in all necessary subfolders
funcpath = which(mfilename);%'G:\Shared drives\INVent\jacob\Code\classification\nFold_cross_validation\nFoldCV_withFS_v3A.m'
%a = funcpath(1:end)
%b = length(mfilename)
funcdir = funcpath(1:end-length(mfilename));%'G:\Shared drives\INVent\jacob\Code\classification\nFold_cross_validation\nF'
idx1 = strfind(funcdir,'\');%3    17    24    30    35    50    73 
idx2 = strfind(funcdir,'/');%[]
if isempty(idx1)
disp('idx2')
