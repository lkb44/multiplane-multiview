function ploth = feature_map_slice(img,mask,featvec)
...
v95 = prctile(featslice(~isnan(featslice)),98);
featslice(featslice>v95) = v95;
% and all things less than 5th percentile to that value
v5 = prctile(featslice(~isnan(featslice)),2);
featslice(featslice<v5) = v5;


% transpose or not? change that here!
img = transpose(img);
mask = transpose(mask);
featslice = transpose(featslice);
