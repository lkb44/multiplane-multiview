...
train_matrix_path = "/Users/leobao/Documents/MultiPlanePipeline/2023-SPIE/Data/AxialTrainingTextureFeatures/";
test_matrix_path = "/Users/leobao/Documents/MultiPlanePipeline/2023-SPIE/Data/AxialTestingTextureFeatures/";

feature_column_names = readtable('/Users/leobao/Documents/MultiPlanePipeline/2023-SPIE/column_feature_names.xlsx', 'ReadVariableNames',false);

% Create cell array with names of each feature
feature_column_names = table2cell(feature_column_names);

% if strcmp(rois, 'Proximal_Fat')
%     feature_column_path = '/Users/leobao/Documents/MultiPlanePipeline/2023-SPIE/2D_Shape_Feature_Extraction_Code/2d_shape_feature_names_proxfat.xlsx';
...
% matrix_path = "/Users/leobao/Documents/MultiPlanePipeline/2023-SPIE/Data/AxialTrainShapeFeatures";
% matrix_path = strcat(matrix_path, view, cohort, 'ShapeFeatures/');
...
output_path = strcat(output_path, view, 'TrainingTextureFeatures/', experiment_date, "_", view, "_", experiment_type, "_", scheme, "_", extraparams, "/");
...
fat_train = find(contains(train,'Axial_Fat_2D'));
proxfat_train = find(contains(train,'_Proximal_Fat_2D'));
tumor_train = find(contains(train,'_Tumor_2D'));

test_matrices = dir(fullfile(test_matrix_path));
test = {test_matrices.name};

fat_test = find(contains(test,'Axial_Fat_2D'));
proxfat_test = find(contains(test,'_Proximal_Fat_2D'));
tumor_test = find(contains(test,'_Tumor_2D'));

for i = 1:length(rois)
    if strcmp(rois(i), 'Fat')
        path_to_train = strcat(train_matrix_path, train{fat_train});
    end
    if strcmp(rois(i), 'Proximal_Fat')
        path_to_train = strcat(train_matrix_path, train{proxfat_train});
    end
    ...
    if strcmp(rois(i), 'Proximal_Fat')
        path_to_test = strcat(test_matrix_path, test{proxfat_test});
    end
    if strcmp(rois(i), 'Tumor')
...
elseif(length(view) == 1 && experiment_type == "proxfat_only")
    features_training = datasets.train_Proximal_Fat.feature_matrix;
    %features_training(:,14) = [];
    features_holdout = datasets.test_Proximal_Fat.feature_matrix;
    %features_holdout(:,14) = [];
    feature_column_names = strcat(view{1}, "_proxfat_", feature_column_names);
end
