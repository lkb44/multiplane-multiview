...
%
addpath('../classifiers')
addpath('../training_and_testing_sets')
if nargin < 6
    Subsets = {};
end
if nargin < 5
...
for j=1:nIter
    ...
    decision  = -inf*(ones(size(data_labels)));
    prediction = -inf*(ones(size(data_labels)));
    ...
    for i=1:n
        ...
        % ***************************%
        [temp_stats,methodstring] = Classify_wrapper( 'QDA', training_set , testing_set, training_labels(:), testing_labels(:));
    ...
    if numel(unique(data_labels))>1 %numel(unique(testing_labels))>1
        if n == 1
        [FPR,TPR,T,AUC,OPTROCPT,~,~] = perfcurve(data_labels(tes{i}),prediction(tes{i}),1,'XVals', [0:0.02:1]);
        else
        [FPR,TPR,T,AUC,OPTROCPT,~,~] = perfcurve(data_labels,prediction,1,'XVals', [0:0.02:1]);
        end
        stats(j).AUC = AUC;
        stats(j).TPR = TPR;
        stats(j).FPR = FPR;
