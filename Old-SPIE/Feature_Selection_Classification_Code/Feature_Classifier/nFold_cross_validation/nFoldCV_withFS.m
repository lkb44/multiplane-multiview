function [stats]= nFoldCV_withFS(data_set,data_labels,params)
...
%                   params.simplewhiten: (optional) logical of whether to apply simplewhitening to the data -- options: true, false (Default: true)
%                   params.fsprunecorr: (optional) logical of whether to applying correlation-based pruning per feature family to initial feature set before embedded feature selection -- options: true, false (Default: false)
%                   params.featnames: (optional) cell array of feature names for each feature in "data_set" (REQUIRED if params.fsprune = true)
%
% OUTPUTS:
...
% 08-07-2019 - Added params field "simplewhiten", "fsprunecorr",
% "featnames". By Default, simplewhitening will always be applied to data_set; pruning correlated features per family is turned off
...
% params.classifier = 'LDA'; %default
% params.classifieroptions = {}; %default
% params.fsname = 'wilcoxon'; %default
% params.shuffle = 1; %default
% params.n = 3; %default
% params.nIter = 25; %default
% params.num_top_feats = 1; %default
% params.feature_idxs = 1:size(data_set,2); %default
% params.threshmeth = 'euclidean'; %default
% params.classbalancetestfold ='none'; %default
% params.simplewhiten = true; %default
% params.fsprunecorr = false; %default

% stats = nFoldCV_withFS(data_set,data_labels,params);
%===============================================

%% Check for valid inputs
if isa(data_labels,'cell'), error('[nFoldCV_withFS:] DATA_LABELS must be a non-cell array with labels 1 and -1.'); end
if all([any(~xor(data_labels == 1, data_labels == -1)), any(~xor(data_labels == 1, data_labels == 0))]), error('[nFoldCV_withFS:] DATA_LABELS must be 1 and -1 (or 1 and 0)'); end
if size(data_set,1)~=length(data_labels), error('[nFoldCV_withFS:] DATA_LABELS must contain same number of labels as observations in DATA_SET (equal # of rows)'); end
if nargin==3 && ~isstruct(params), error('[nFoldCV_withFS:] PARAMS must be a struct'); end    
if isrow(data_labels), data_labels = transpose(data_labels); end %needed for class balancing operations

%%  Set default inputs

%add in all necessary subfolders
funcname = 'nFoldCV_withFS.m';
...
if nargin == 3
    ...
    if ~isfield(params,'simplewhiten')         || isempty(params.simplewhiten),    params.simplewhiten = true;                end
    if ~isfield(params,'fsprunecorr')          || isempty(params.fsprunecorr),     params.fsprunecorr = false;       end
end  

%% nFold CV with FS
stats = struct; %in the end: contains (params.nIter)-rows of information ;

if params.simplewhiten
    data_set = simplewhiten(data_set);
end

% if nans in data, need to be removed for feature selection!
if any(isnan(data_set(:)))
    error('[nFoldCV_withFS:] NaNs exist in the dataset. Please check. Then remove these observations/features. Some feature selection techniques cannot handle nans.');
end

if params.fsprunecorr
    if ~isfield(params,'featnames') || isempty(params.featnames) || length(params.featnames)~=size(data_set,2)
        error('[nFoldCV_withFS:] Must assign a valid cell array of strings to "params.featnames" if params.fsprunecorr = true');
    else
        if length(params.feature_idxs)<size(data_set,2)
            warning('Pruning Correlated Features Per Family -- will override current "params.feature_idxs"');
        end
        %get the indices of the first instance of each feature family.
        %****EDIT THIS AS MORE FEATURE FAMILIES ARE ADDED IN THE FUTURE***%
        gray_idx = min([find(contains(params.featnames,'Raw'),1), find(contains(params.featnames,'Gray'),1)]);  
        grad_idx = find(contains(params.featnames,'Grad'),1);
        har_idx = find(contains(params.featnames,'Haralick'),1);
        gab_idx = find(contains(params.featnames,'Gabor'),1);
        law_idx = find(contains(params.featnames,'Laws'),1);
        collage_idx = find(contains(params.featnames,'CoLlAGe'),1);
        
        feat_family_idx = [gray_idx, grad_idx, har_idx, law_idx, collage_idx];
        
        params.feature_idxs = pick_best_uncorrelated_features_perfamily(data_set,data_labels,feat_family_idx);
    end
