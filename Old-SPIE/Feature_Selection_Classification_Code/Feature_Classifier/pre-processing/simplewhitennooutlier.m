function [whiteneddata,mean_vec,mad_vec,outfreq,idx] = simplewhitennooutlier(data, varargin)
datanoout={};
...
if nargin>2
   ...
   datanoout = cell2mat(datanoout);
   
   mean_vec = varargin{1,1};
   temp = bsxfun(@minus,datanoout,mean_vec);			%subtract means
   
   mad_vec = varargin{1,2};
   if sum(mad_vec)>=0								%checking for any MAD=0
    ...
    data_whiten = bsxfun(@minus,data,mean_vec); % substract means
    data_whiten = bsxfun(@rdivide,data_whiten,mad_vec); % divide by MAD
    whiteneddata = data_whiten;
    return;
