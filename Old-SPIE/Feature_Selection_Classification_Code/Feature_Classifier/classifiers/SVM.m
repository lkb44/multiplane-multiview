function [methodstring,stats] = SVM( training_set , testing_set, training_labels, testing_labels, options)
...
%  and then all other fitcsvm Name-Value pairs
%  (i.e. 'KernelFunction',...)
...
idx = find(strcmpi(options,'KernelFunction'));
if isempty(idx), options = cat(2,options,{'KernelFunction', 'gaussian'}); end

idx = find(strcmpi(options,'OptimizeHyperparameters'));
if isempty(idx), options = cat(2,options,{'OptimizeHyperparameters', 'auto'}); end

idx = find(strcmpi(options,'HyperparameterOptimizationOptions'));
if isempty(idx), options = cat(2,options,{'HyperparameterOptimizationOptions',struct('ShowPlots',false,'Verbose',0)}); end
...
Mdl = fitcsvm(training_set,training_labels,options{:});
