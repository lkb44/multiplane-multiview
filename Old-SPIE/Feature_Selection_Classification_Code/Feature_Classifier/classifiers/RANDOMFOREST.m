function [methodstring,stats, Mdl] = RANDOMFOREST( training_set , testing_set, training_labels, testing_labels, options )
...
idx = find(strcmpi(options,'Model'));
if ~isempty(idx), Mdl = options{idx+1}; end
options(idx:idx+1) = []; options = squeeze(options);
