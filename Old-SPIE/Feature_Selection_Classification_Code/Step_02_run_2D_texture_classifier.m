...
addpath(genpath("/Users/leobao/Library/CloudStorage/GoogleDrive-lkb44@case.edu/Shared drives/INVent/tom/Conferences/2022-SPIE/Feature_Selection_Classification_Code/mrmr_feature_select/"));
%% Specify dataset information
view = {'Axial'}; % {Can only be 'Axial'}, {'Coronal'} or {'Axial','Coronal'}
rois = {'proxfat', 'fat', 'tumor'};
...
if(length(view) == 1 && view{1} == "Axial")
    train_matrix_path = "../Texture_Features/2D Features/Axial/train/";
    test_matrix_path = "../Texture_Features/2D Features/Axial/test/";
    va_test_matrix_path = "../Texture_Features/2D Features/Axial/va_test/";
elseif(length(view) == 1 && view{1} == "Coronal")
    train_matrix_path = "../Texture_Features/2D Features/Coronal/train/"; 
    test_matrix_path = "../Texture_Features/2D Features/Coronal/test/";
    va_test_matrix_path = "../Texture_Features/2D Features/Coronal/va_test/";
elseif(length(view) == 2)
    ...
    ax_train_matrix_path = "../Texture_Features/2D Features/Axial/train/";
    ax_test_matrix_path = "../Texture_Features/2D Features/Axial/test/";
    ax_va_test_matrix_path = "../Texture_Features/2D Features/Axial/va_test/";
    cor_train_matrix_path = "../Texture_Features/2D Features/Coronal/train/"; 
    cor_test_matrix_path = "../Texture_Features/2D Features/Coronal/test/";
    cor_va_test_matrix_path = "../Texture_Features/2D Features/Coronal/va_test/";
end

% Create cell array with names of each feature
feature_column_names = load("../Texture_Features/2D Features/Axial/train/StatName.mat");
feature_column_names = feature_column_names.nameStat;

fprintf("Got directory paths to feature matrices! \n");

%% Create output directory

output_path = "/Volumes/GoogleDrive/Shared drives/INVent/tom/Conferences/2022_SPIE/Texture_Feature_Results/";
...
if(length(view) == 1)
    for i=1:length(rois)
        path_to_train = strcat(train_matrix_path, view{1}, "_", rois{i}, "_train_2D_texture_features.mat");
    ...
    for i=1:length(rois)
        ...
        % Axial datasets
        path_to_train = strcat(ax_train_matrix_path, view{1}, "_", rois{i}, "_train_2D_texture_features.mat");
        datasets.(strcat(view{1}, "_train_",rois{i})) = load(path_to_train);
        
        path_to_test = strcat(ax_test_matrix_path, view{1}, "_", rois{i}, "_test_2D_texture_features.mat");
        datasets.(strcat(view{1}, "_test_",rois{i})) = load(path_to_test);
        
        path_to_test = strcat(ax_va_test_matrix_path, view{1}, "_", rois{i}, "_va_test_2D_texture_features.mat");
        datasets.(strcat(view{1}, "_va_test_",rois{i})) = load(path_to_test);
        
        % Coronal Datasets
        path_to_train = strcat(cor_train_matrix_path, view{2}, "_", rois{i}, "_train_2D_texture_features.mat");
        datasets.(strcat(view{2}, "_train_",rois{i})) = load(path_to_train);
        
        path_to_test = strcat(cor_test_matrix_path, view{2}, "_", rois{i}, "_test_2D_texture_features.mat");
        datasets.(strcat(view{2}, "_test_",rois{i})) = load(path_to_test);
        
        path_to_test = strcat(cor_va_test_matrix_path, view{2}, "_", rois{i}, "_va_test_2D_texture_features.mat");
...
elseif(length(view) == 1 && experiment_type == "all") % Single view with all lumne, rectal wall, and fat features
    ...
    if(strcmp(view{1}, 'Axial'))
       
       %lumen_train = datasets.train_lumen.feature_matrix; 
       rw_train = datasets.train_tumor.feature_matrix;
       fat_train = datasets.train_fat.feature_matrix;
       features_training = [rw_train fat_train];

       %lumen_test = datasets.test_lumen.feature_matrix;
       rw_test = datasets.test_tumor.feature_matrix;
       fat_test = datasets.test_fat.feature_matrix;
       features_holdout = [rw_test fat_test];

       %va_lumen_test = datasets.va_test_lumen.feature_matrix;
       va_rw_test = datasets.va_test_tumor.feature_matrix;
       va_fat_test = datasets.va_test_fat.feature_matrix;
       features_va = [va_rw_test va_fat_test];
       features_holdout = vertcat(features_holdout, features_va);
...
fprintf(fileid, "FEATURE FAMILY: 3D Texture \n");
