...
problems = [];
pathIn = '/Users/leobao/Documents/MultiPlanePipeline/2023-SPIE/Data/CoronalSingle/';

DirectoryIn= '/Users/leobao/Documents/MultiPlanePipeline/2023-SPIE/Data/CoronalSingle/UH-RectalCA-*';
pathOut= '/Users/leobao/Documents/MultiPlanePipeline/2023-SPIE/Data/CoronalOut/';

Imgs = dir(fullfile(DirectoryIn));
theFilesvol = {Imgs.name};

for k= 1:length(theFilesvol)
    folderIn = dir(fullfile(pathIn, theFilesvol{k}));
    if size(folderIn,1)==0, continue; end
    P = {folderIn.name};
    
    img_index = find(~contains(P,'Cor.mha'));
    mask_index = find(contains(P,'Segmentation.mha'));

    img_file_in = '/Users/leobao/Documents/MultiPlanePipeline/2023-SPIE/Data/CoronalSingle/UH-RectalCA-066/RectalCA066_Pre_Cor.mha';
...
myinterp = 'linear'; %linear, for T2w MRI sequences, anyways, CHANGE THIS FOR MASK
...
if nargin > 5 && ~isempty(mask_file_in)
    ...
    interpolator = ['-i ' myinterp ' '];
    spacing = ['-s ' num2str(newXres) ',' num2str(newYres) ',' num2str(newZres) ' '];  %new X, Y, Z resolutiosn
    mask_out_file = fullfile(out_dir,[name '_aniresampled' ext]); %img_file_out is same name as in img_file_in, but with "_resampled" tag
