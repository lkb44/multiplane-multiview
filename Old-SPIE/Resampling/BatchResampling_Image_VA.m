...
problems = [];
pathIn = '../Data/Original_Coronal/VA/';
% path ='G:\Shared drives\INVent\amir\Rectal\UH_preCRT_Resampled';
DirectoryIn= '../Data/Original_Coronal/VA/VA_RectalCA_*';
pathOut= '../Data/Coronal_resampled/';
Imgs = dir(fullfile(DirectoryIn));
theFilesvol = {Imgs.name};
for k= 1:length(theFilesvol)
    folderIn = dir(fullfile(pathIn, theFilesvol{k}, 'VA_RectalCA_*'));
    if size(folderIn,1)==0, continue; end
    P = {folderIn.name};
    
    img_index = find(~contains(P,'mask') & contains(P, 'cor'));
    mask_index = find(contains(P,'mask') & contains(P, 'cor'));
