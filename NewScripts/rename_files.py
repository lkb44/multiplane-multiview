...
data_path = "/Users/leobao/Documents/MultiPlanePipeline/Patients/"
...
for folder in dir_list:
    ...
    for old_name in files:
        if "label" not in old_name and "DS" not in old_name:
            new_name = old_name.replace(old_name, folder + "_cor.mha")
        if "segmentation" in old_name or "Segmentation" in old_name or "mask" in old_name or "label" in old_name:
            new_name = old_name.replace(old_name, folder + "_cor_label.mha")
