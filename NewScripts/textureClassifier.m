...
training_label_path = "/Volumes/Crucial X6/train_labels_unofficial.csv";
testing_label_path = '/Volumes/Crucial X6/test_labels_unofficial.csv';
train_matrix_path = "/Volumes/Crucial X6/CombineTexture/Axial_Training/";
test_matrix_path = "/Volumes/Crucial X6/CombineTexture/Axial_Testing/";
...
output_path = strcat(output_path, view, 'TrainingTextureFeatures', split, '/', experiment_date, "_", view, "_", split, "_", experiment_type, "_", scheme, "/");
...
for i = 1:length(rois)
    ...
    if strcmp(rois(i), 'Tumor')
        path_to_train = strcat(train_matrix_path, view, '_', rois, '.mat');
    end
    ...
    if strcmp(rois(i), 'Tumor')
        path_to_test = strcat(test_matrix_path, view, '_', rois, '.mat');
    end
...
%     features_holdout = features_holdout(:, nan_indices_train);
%     feature_column_names = feature_column_names(:, nan_indices_train);
%     nan_vector_test = features_holdout(1, :);
%     nan_indices_test = find(~isnan(nan_vector_test));
%     features_training = features_training(:, nan_indices_test);
%     features_holdout = features_holdout(:, nan_indices_test);
%     feature_column_names = feature_column_names(:, nan_indices_test);
% 
%     inf_vector_train = features_training(1, :);
%     inf_indices_train = find(~isinf(inf_vector_train));
%     features_training = features_training(:, inf_indices_train);
%     features_holdout = features_holdout(:, inf_indices_train);
%     feature_column_names = feature_column_names(:, inf_indices_train);
%     inf_vector_test = features_holdout(1, :);
%     inf_indices_test = find(~isinf(inf_vector_test));
%     features_training = features_training(:, inf_indices_test);
%     features_holdout = features_holdout(:, inf_indices_test);
%     feature_column_names = feature_column_names(:, inf_indices_test);
